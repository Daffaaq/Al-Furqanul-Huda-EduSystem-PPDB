            ...
            <div class="card-body">
                <div class="table-responsive">
                    <div class="d-flex flex-wrap justify-content-between align-items-end mb-4">
                        ...
                        @can('biodata-calon-siswa.create')
                            <div class="mt-3 mt-sm-0">
                                <a href="{{ route('biodata-calon-siswa.create') }}" class="btn btn-sm btn-primary shadow-sm">
                                    <i class="fas fa-plus fa-sm text-white-50"></i> Tambah Biodata
                                </a>
                            </div>
                        @endcan
                    </div>
                    ...
                    <table class="table table-bordered" id="biodataCalonSiswaTable" width="100%" cellspacing="0">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Nama</th>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            let table = $('#biodataCalonSiswaTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('biodata-calon-siswa.list') }}",
                    type: 'POST',
                    data: function(d) {
                        d._token = '{{ csrf_token() }}';
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            let editUrl =
                                `/pendaftaran-management/biodata-calon-siswa/${data}/edit`;
                            let showUrl = `/pendaftaran-management/biodata-calon-siswa/${data}`;
                            let deleteUrl = `/pendaftaran-management/biodata-calon-siswa/${data}`;
                            return `
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $('#biodataCalonSiswaTable').DataTable().ajax.reload();
                            } else {
